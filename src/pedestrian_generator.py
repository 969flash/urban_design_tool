...
BLOCKS = list(globals().get("blocks", []) or [])  # List[Block]
...
for blk in BLOCKS:
    try:
        if getattr(blk, "landuse_name", None) in EXCLUDE_LANDUSES:
            continue
        region_brep = getattr(blk, "region", None)
        ...
        _dprint("Block failed:", getattr(blk, "block_id", None), e)
